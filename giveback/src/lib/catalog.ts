// Reference data shared by every screen: what can be given, in what
// condition, and where. The ids match the database check constraints.

import type { ItemCondition } from './types';

export const CATEGORIES = [
  { id: 'furniture', label: 'רהיטים', icon: 'bed-outline' },
  { id: 'baby', label: 'תינוקות וילדים', icon: 'happy-outline' },
  { id: 'toys', label: 'צעצועים', icon: 'extension-puzzle-outline' },
  { id: 'books', label: 'ספרים', icon: 'book-outline' },
  { id: 'home', label: 'ציוד לבית', icon: 'home-outline' },
  { id: 'electronics', label: 'חשמל', icon: 'tv-outline' },
  { id: 'clothing', label: 'ביגוד', icon: 'shirt-outline' },
  { id: 'sports', label: 'ספורט ופנאי', icon: 'bicycle-outline' },
  { id: 'garden', label: 'גינה', icon: 'leaf-outline' },
  { id: 'food', label: 'אוכל', icon: 'nutrition-outline' },
...
export const CONDITIONS: { id: ItemCondition; label: string }[] = [
  { id: 'new', label: 'חדש' },
  { id: 'like_new', label: 'כמו חדש' },
  { id: 'good', label: 'מצב טוב' },
  { id: 'fair', label: 'צריך תיקון קל' },
];

export function conditionLabel(id: string | null | undefined) {
  return CONDITIONS.find((c) => c.id === id)?.label ?? '';
}

export const COMMUNITY_KINDS = [
  { id: 'neighborhood', label: 'שכונה' },
  { id: 'building', label: 'בניין' },
  { id: 'school', label: 'בית ספר / גן' },
  { id: 'kibbutz', label: 'קיבוץ / מושב' },
  { id: 'workplace', label: 'מקום עבודה' },
  { id: 'other', label: 'אחר' },
] as const;

export const REPORT_REASONS = [
  { id: 'scam', label: 'הונאה / מבקש/ת כסף' },
  { id: 'prohibited', label: 'פריט אסור' },
  { id: 'offensive', label: 'תוכן פוגעני' },
  { id: 'spam', label: 'ספאם' },
  { id: 'no_show', label: 'לא הגיע/ה לאיסוף' },
  { id: 'other', label: 'אחר' },
] as const;

export const RADII = [1, 3, 5, 10, 25] as const;

// Municipal centres, used when someone picks a city instead of sharing GPS.
export const CITIES = [
