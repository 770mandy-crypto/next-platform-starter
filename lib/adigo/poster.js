...
export async function drawPoster({ campaign, business, palette = "חם", badge = "" }) {
  ...
  // כמה שורות גוף שנכנסות עד כפתור הסיום, כדי שלא יתנגשו
  const ctaTop = SIZE - MARGIN - 158;
  const roomLines = Math.floor((ctaTop - y - 20) / 58);

  ctx.fillStyle = p.sub;
  ctx.font = "400 40px Assistant, Arial, sans-serif";
  const bodyText = String(campaign.body || "").replace(/\n+/g, " ");
  for (const line of wrapLines(ctx, bodyText, maxWidth, Math.max(1, Math.min(5, roomLines)))) {
    ctx.fillText(line, right, y);
    y += 58;
  }
