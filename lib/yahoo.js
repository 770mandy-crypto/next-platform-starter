...
// The whole serverless request has roughly ten seconds. Yahoo has to fail fast
// enough to leave the fallback provider time to answer, so its budget is a
// small slice of that rather than the entire thing.
const YAHOO_TIMEOUT_MS = Number(process.env.YAHOO_TIMEOUT_MS) || 3000;

async function fetchJson(url, { timeoutMs = YAHOO_TIMEOUT_MS, headers = BROWSER_HEADERS } = {}) {
...
async function getSessionCookie() {
    const attempts = [];
    // The hosts share one budget rather than getting the full timeout each:
    // three sequential hangs would otherwise consume the whole serverless
    // request on their own.
    const deadline = Date.now() + YAHOO_TIMEOUT_MS;

    for (const source of COOKIE_SOURCES) {
        const remaining = deadline - Date.now();
        if (remaining <= 0) {
            attempts.push(`${source} → skipped (budget spent)`);
            continue;
        }

        const { signal, done } = withTimeout(remaining);
        try {
...
async function getCrumb({ force = false } = {}) {
    ...
    const { signal, done } = withTimeout(YAHOO_TIMEOUT_MS);
    try {
