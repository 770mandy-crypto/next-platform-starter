    ...
    return (
        <Link href={`/product/${product.slug}`} className="group flex flex-col">
            ...
            >
                ...
                {product.badge && (
                    <span
                        className="absolute top-4 px-3 py-1.5 text-[0.6rem] font-semibold tracking-[0.18em] uppercase start-4"
                ...
                <span
                    className="absolute inset-x-0 bottom-0 py-3 text-[0.62rem] font-semibold tracking-[0.26em] text-center uppercase translate-y-full transition-transform duration-500 group-hover:translate-y-0"
                    style={{ background: 'var(--color-gold)', color: 'var(--color-ink)' }}
                    aria-hidden="true"
                >
                    צפייה בפריט
            ...
            <div className="flex items-baseline justify-between gap-4 mt-5">
                ...
                <p className="flex items-baseline gap-2 shrink-0 tabular-nums">
                    {product.compareAtPrice && (
                        <span className="text-xs line-through text-muted">{formatPrice(product.compareAtPrice)}</span>
                    )}
